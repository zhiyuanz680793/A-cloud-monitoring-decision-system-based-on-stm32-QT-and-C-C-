...
int main(void)
{
	...
	OLED_ShowString(4,5,"R_D:");//USART接收到数据
	AD_Init();
	...
	while (1)
	{
		...
		ADC_t = (AD_Value[0] + AD_Value[1] + AD_Value[2] + AD_Value[3]) / 4;	
		...
		if (Serial_GetRxFlag() == 1)			//检查串口接收数据的标志位
		{
			...
			OLED_ShowHexNum(4, 10, Rx_Data, 4);	//显示串口接收的数据
		}	
